...
3. WS/TLS/订阅主机设置 Proxied；Reality 使用 DNS only 的独立主机或源站 IP。
...
验证脚本只打印状态、长度、内容类型和结构标记，不打印完整订阅正文或 Token。

门禁：profile/provider 两个端点都是 200、非 HTML、无 Challenge，并包含预期 YAML 结构。
